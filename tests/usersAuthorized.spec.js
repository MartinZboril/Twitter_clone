...
test.serial(
  "POST /logout will remove the user token",
  async (t) => {
    ...
    const cookiesAfterLogout =
      responseLogout.headers["set-cookie"]
    const tokenCookieAfterLogout =
      cookiesAfterLogout &&
      cookiesAfterLogout.find((cookie) =>
...
test.serial(
  "GET /edit profile shows form for update user profile",
  async (t) => {
...
test.serial(
  "POST /edit profile will update user",
  async (t) => {
    await agent.post("/edit-profile").type("form").send({
      email: "email",
      name: "updated name",
      bio: "bio",
    })
...
test.serial(
  "after profile editing and redirect user name is updated",
  async (t) => {
...
test.serial(
  "GET /change password shows form for update user password",
  async (t) => {
...
test.serial(
  ...
  async (t) => {
    ...
    await agent.post("/logout").type("form")
